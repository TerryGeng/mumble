...
void TestAES256GCMCrypt::authcrypt() {
	for (int len = 1; len < 32; len++) {
		unsigned char rawkey[256 / 8] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		                              0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		                              0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };

		unsigned char iv[96 / 8] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		                             0x08, 0x09, 0x0a, 0x0b};
...
void TestAES256GCMCrypt::ivrepeat() {
	...
	unsigned char iv[96 / 8] = { 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
				     0x08, 0x09, 0x0a, 0x0b};
	std::string rawkey_str = std::string(reinterpret_cast< const char * >(rawkey), 256 / 8);
	std::string nonce_str  = std::string(reinterpret_cast< const char * >(iv), 96 / 8);
	CryptStateAES256GCM cs;
